...
public class SockServer {
  ...
  static List<String> storyboardSentences = new ArrayList<>();
  static List<String> storyboardUsers = new ArrayList<>();
  ...
  static JSONObject storyboard(JSONObject req) {
    ...
    // Check if it's a request to view the storyboard
    if (req.optBoolean("view", false)) {
      JSONArray sentences = new JSONArray(storyboardSentences);
      JSONArray users = new JSONArray(storyboardUsers);
      ...
      String story = req.optString("story", null);
      
      if (name == null || story == null || name.isEmpty() || story.isEmpty()) {
        res.put("ok", false);
        res.put("message", "Username and story must be provided.");
      } else if (storyboardUsers.contains(name)) {
        res.put("ok", false);
        res.put("message", "This user has already added to the storyboard.");
      } else {
        // Add the story and username to their respective lists
        storyboardSentences.add(story);
        storyboardUsers.add(name);
        res.put("ok", true);
      }
    }
    return res;
  ...
  static JSONObject charCount(JSONObject req) {
    ...
    JSONObject fieldTest = testField(req, "count");
    if (!fieldTest.getBoolean("ok"))
      return fieldTest;

    String count = req.getString("count");
    if (req.getBoolean("findchar")) {
      // Specific character count
      fieldTest = testField(req, "find");
      if (!fieldTest.getBoolean("ok"))
        return fieldTest;

      String find = req.getString("find");
      long charCount = count.chars().filter(ch -> ch == find.charAt(0)).count();
